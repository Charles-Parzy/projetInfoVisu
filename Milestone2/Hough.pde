ArrayList<PVector> hough(PImage edgeImg, int nLines) {
  ...
  int rDim = (int) (((result.width + result.height) * 2 + 1) / discretizationStepsR);
  ...
  /*********************************************************
  * Construct the accumulator (TEMPORARY FOR MILESTONE #2)
  **********************************************************/
  acc = createImage(rDim + 2, phiDim + 2, ALPHA);
  for (int i = 0; i < accumulator.length; i++) {
    acc.pixels[i] = color(min(255, accumulator[i]));
  }
  acc.resize(400, 300);
  acc.updatePixels();
